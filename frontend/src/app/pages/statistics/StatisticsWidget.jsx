  ...
  return (
    <DashboardWidgetShell loading={isLoading} size="sm" transparent={true}>
      <div className={`stats-grid stats-grid--${showScenes ? '5' : '4'}`}>
        ...
        {showScenes && (
          <div className="stat-card">
            <div className="stat-label">{T('statistics.stats.total_scenes') || 'Total Scenes'}</div>
            <div className="stat-value">{(stats.total_scenes || 0).toLocaleString()}</div>
            <div className="stat-sub">{T('statistics.stats.scenes_sub') || 'In Library'}</div>
          </div>
        )}
        <div className="stat-card">
          <div className="stat-label">{T('statistics.stats.tv_series') || 'TV Shows'}</div>
          <div className="stat-value">{(stats.total_tv || 0).toLocaleString()}</div>
          <div className="stat-sub">
            {(stats.total_episodes || 0).toLocaleString()} {T('statistics.stats.episodes_sub') || 'Episodes'}
          </div>
        </div>
        <div className="stat-card">
          ...
          <div className="stat-sub">
            {T('statistics.stats.storage_sub', { count: stats.drive_count || 0 }) || `${stats.drive_count || 0} drives connected`}
          </div>
        </div>
        <div className="stat-card">
