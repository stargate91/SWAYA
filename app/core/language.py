...
def get_user_ui_language(settings, user_id=None) -> str:
    ...
    lang = settings.get_setting("primary_metadata_language", user_id)
    return LanguageService.clean_locale(lang) if lang else DEFAULT_FALLBACK_LANGUAGE

def get_user_fallback_language(settings, user_id=None) -> str:
    """Returns normalized fallback metadata language or default fallback ('en')."""
    lang = settings.get_setting("fallback_metadata_language", user_id)
