    ...
    def add_from_pool(pool: List[dict], max_self_like: Optional[int] = None):
        ...
        for credit in pool:
            if len(selected) >= limit:
                break
            cid = credit.get("id")
            parsed_id = int(cid) if isinstance(cid, int) or (isinstance(cid, str) and cid.isdigit()) else (cid or 0)
            credit_key = (parsed_id, str(credit.get("media_type") or credit.get("type") or ""))
...
def resolve_person_known_for_backdrop(
    ...
    ranked_credits = list(credits or []) if respect_credit_order else sorted(
        ...
        key=lambda credit: (
            ...
            get_year_from_date(credit.get("release_date") if credit.get("media_type") == "movie" else credit.get("first_air_date")) or 0,
        ),
        reverse=True,
    )
