...
from app.modules.users.services.overrides.lock_validator import LockValidator
...
class TitleLockService:
    ...
    def update_item_overrides(self, request: ItemOverridesUpdate) -> Dict[str, Any]:
        ...
        if request.is_watched is not None:
            ...
            if is_watched_val:
                ...
                has_active_interaction = True
                self._track_parent_tv_show_if_episode(db, str(item_id), media_item_id, metadata_match_id)
                if media_item_id:
    ...
    def bulk_watched(self, request: BulkWatchedUpdate) -> Dict[str, Any]:
        ...
        parsed_date = parse_datetime_utc(watched_at)
            ...
            if override:
                ...
                if is_watched:
                    ...
                    if is_tv and match:
                        ...
                        self._track_parent_tv_show_if_episode(db, str(item_id), resolved_media_item_id, override.metadata_match_id, tracked_parent_ids)
                else:
